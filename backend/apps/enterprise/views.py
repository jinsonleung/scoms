...
class EnterpriseDetail(mixins.RetrieveModelMixin, mixins.UpdateModelMixin, mixins.DestroyModelMixin,
    ...
    def put(self, request, *args, **kwargs):
        ...
        pk = kwargs.get('pk')
        try:
            current_object = Enterprise.objects.filter(pk=pk, is_delete=False).first()
        ...
        obj_serial = EnterpriseSerializer(instance=current_object, data=request_data)
        obj_serial.is_valid(raise_exception=True)
        new_object = obj_serial.save()
        return Response({
            'status': 0,
            'msg': 'ok',
            'results': EnterpriseSerializer(instance=new_object).data
        })
    ...
    def patch(self, request, *args, **kwargs):
        ...
        book_obj_list = []
        for i in pks:
            current_object = Enterprise.objects.filter(pk=i, is_delete=False).first()
            if not current_object:
                return Response({
                    'status': 1,
                    'msg': '数据错误'
                })
            book_obj_list.append(current_object)
